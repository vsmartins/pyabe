...
You should have read the {ref}`lecture <getting_started>` on getting
started with Python before beginning this one.
...
3. a file called `__init__.py` that specifies what will be executed when
   we type `import package_name`
...
Let\'s try writing some alternative versions of {ref}`our first program
<ourfirstprog>`, which plotted IID draws from the normal distribution.
...
- The next three lines are the `for` loop, which repeatedly draws a new
  random number $\epsilon_t$ and appends it to the end of the list
  `ϵ_values`.
...
and the third is a
[Boolean value](https://en.wikipedia.org/wiki/Boolean_data_type).
...
- Python objects such as lists, strings, etc. all have methods that are
  used to manipulate the data contained in the object.
- String objects have
...
Now let\'s consider the `for` loop from {ref}`the program above
<firstloopprog>`, which was
...
More on indentation below---for now, let\'s look at another example of a
`for` loop
...
- removes clutter, such as the brackets or end statements used in other
  languages
...
- The line before the start of a code block always ends in a colon
  ...
  - `while x < 100:`
  - etc., etc.
- All lines in a code block **must have the same amount of
  indentation**.
...
But, for the purpose of illustration, let\'s modify {ref}`the program
above <firstloopprog>` to use a `while` loop instead.
...
- the code block for the `while` loop is again delimited only by
  indentation
...
In the code below, we generate and plot the sequence $b_0, b_1, \ldots,
b_T$ generated by {eq}`ilom`.
...
$$ x_{t+1} = \alpha \, x_t + \epsilon_{t+1} \quad \text{where} \quad x_0
= 0 \quad \text{and} \quad t = 0,\ldots,T $$
...
- If you call the `plot()` function multiple times before calling
  `show()`, all of the lines you produce will end up on the same figure.
...
$$ x_{t+1} = \alpha \, |x_t| + \epsilon_{t+1} \quad \text{where} \quad
x_0 = 0 \quad \text{and} \quad t = 0,\ldots,T $$
...
The task is to compute an approximation to $\pi$ using
...
- If $U$ is a bivariate uniform random variable on the unit square $(0,
  1)^2$, then the probability that $U$ lies in a subset $B$ of $(0,1)^2$
  is equal to the area of $B$.
...
But here the point is to compute $\pi$, which we can do by $\pi = A /
r^2$.
